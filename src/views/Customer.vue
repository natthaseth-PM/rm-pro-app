<template>
  <div class="h-screen w-full font-sans bg-gray-50 flex flex-col overflow-hidden text-gray-800 relative">
    ...
    <main class="flex-1 overflow-hidden relative bg-gray-50">
      ...
      <section v-show="currentTab === 'menu'" class="h-full flex flex-col animate-[fadeIn_0.3s_ease-out]">
        ...
        <div class="px-5 py-4 flex gap-2 overflow-x-auto no-scrollbar shrink-0 bg-gray-50">
          <button @click="selectedCategory = 'All'" :class="['px-5 py-2 rounded-full font-bold text-xs whitespace-nowrap transition-colors border shadow-sm', selectedCategory === 'All' ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 border-gray-200 hover:border-primary']">ทั้งหมด</button>
      ...
      <section v-show="currentTab === 'history'" class="h-full overflow-y-auto p-5 pb-32 animate-[fadeIn_0.3s_ease-out] bg-gray-100">
        <div class="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 min-h-[300px] relative">
          ...
          <h2 class="text-xl font-black text-gray-800 mb-6 flex items-center pb-4 border-b border-dashed border-gray-300">
            <div class="w-10 h-10 bg-primary/10 text-primary rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-receipt"></i></div>
            รายการที่สั่งไปแล้ว
          </h2>
          
          <div v-if="isHistoryLoading" class="text-center py-12 text-gray-400"><i class="fa-solid fa-circle-notch fa-spin text-4xl"></i></div>
          <div v-else-if="historyItems.length === 0" class="text-center py-12 text-gray-400 font-bold flex flex-col items-center">
            <i class="fa-regular fa-face-frown-open text-5xl mb-3 opacity-30"></i>
          ...
          <div v-else class="space-y-4">
            <div v-for="(item, idx) in historyItems" :key="idx" class="flex justify-between items-start pb-4 border-b border-gray-50 last:border-0">
              <div class="flex-1 pr-4">
                ...
                <span :class="getStatusStyle(item.kitchen_status).class" class="text-[10px] font-black px-2.5 py-1 rounded-md uppercase tracking-wider flex items-center inline-flex">
                  <i :class="getStatusStyle(item.kitchen_status).icon" class="mr-1"></i> {{ getStatusStyle(item.kitchen_status).text }}
                </span>
              </div>
            ...
            <div class="mt-8 bg-gray-900 text-white p-5 rounded-2xl shadow-lg relative overflow-hidden">
              ...
              <div class="flex justify-between items-center relative z-10">
                <span class="font-bold text-sm text-gray-300 uppercase tracking-widest">ยอดรวมบิลนี้</span>
                <span class="text-3xl font-black text-primary">฿{{ Number(historyTotal).toLocaleString() }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <button @click="showCartSummary = true" class="w-full bg-gray-900 text-white p-4 rounded-2xl shadow-2xl shadow-gray-900/40 flex justify-between items-center active:scale-95 transition-all border border-gray-700">
        <div class="flex items-center gap-4">
          <div class="relative w-12 h-12 bg-gray-800 rounded-xl flex items-center justify-center text-xl text-primary border border-gray-700">
            ...
            <div class="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-black w-6 h-6 rounded-full flex items-center justify-center border-2 border-gray-900 shadow-sm animate-bounce">{{ cartCount }}</div>
          </div>
          <div class="text-left">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">ตะกร้าของคุณ</p>
            <p class="font-black text-2xl leading-none">฿{{ cartTotal.toLocaleString() }}</p>
          </div>
        </div>
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around items-center h-[72px] z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] pb-safe">
      <button @click="currentTab = 'menu'" :class="currentTab === 'menu' ? 'text-primary scale-110' : 'text-gray-400 hover:text-gray-600'" class="flex-1 flex flex-col items-center transition-all duration-200">
        <i class="fa-solid fa-utensils text-2xl mb-1"></i><span class="text-[10px] font-black uppercase tracking-wider">สั่งอาหาร</span>
      </button>
      <button @click="requestService('Staff')" class="flex-1 flex flex-col items-center text-gray-400 hover:text-orange-500 transition-colors group relative">
        <div class="absolute -top-8 bg-white border border-gray-200 shadow-md rounded-full w-14 h-14 flex items-center justify-center group-hover:-translate-y-1 transition-transform">
           <i class="fa-solid fa-bell text-2xl text-orange-400 group-hover:animate-ring"></i>
        </div>
        <span class="text-[10px] font-black uppercase tracking-wider mt-6">เรียกพนักงาน</span>
      </button>
      <button @click="openHistoryTab" :class="currentTab === 'history' ? 'text-primary scale-110' : 'text-gray-400 hover:text-gray-600'" class="flex-1 flex flex-col items-center transition-all duration-200 relative">
        <i class="fa-solid fa-receipt text-2xl mb-1"></i><span class="text-[10px] font-black uppercase tracking-wider">เช็คบิล</span>
        <span v-if="historyItems.length > 0" class="absolute top-0 right-1/4 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span>
      </button>
    </nav>
      ...
      <div class="bg-white w-full rounded-t-[2.5rem] shadow-2xl flex flex-col max-h-[85vh] relative z-10 animate-[slideUp_0.3s_ease-out]">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
          <div v-for="item in cart" :key="item.id" class="flex gap-4 bg-white p-4 rounded-2xl border border-gray-100 items-center shadow-sm">
            <div class="flex-1 min-w-0">
              <h4 class="font-black text-base text-gray-800 mb-1">{{ item.menu_name }}</h4>
              <p class="text-lg font-black text-primary leading-none">฿{{ Number(item.price * item.qty).toLocaleString() }}</p>
            </div>
        ...
        <div class="p-6 bg-white border-t border-gray-100 pb-10">
          ...
          <button @click="submitOrder" :disabled="isSubmitting" class="w-full bg-gray-900 disabled:bg-gray-300 text-white font-black py-5 rounded-2xl shadow-xl shadow-gray-900/30 active:scale-95 transition-all text-xl flex justify-center items-center gap-3">
            ...
            <i v-else class="fa-solid fa-bell-concierge text-primary"></i>
...
const Toast = Swal.mixin({
  toast: true,
  position: 'top', // เด้งด้านบนให้ลูกค้าเห็นชัดๆ
...
const loadInitialData = async () => {
  // รับพารามิเตอร์จาก URL (เช่น /customer?table_id=1&table_name=โต๊ะ1)
  ...
  if (!tableId.value) {
    Swal.fire({ 
      icon: 'error', 
      title: 'ไม่พบรหัสโต๊ะ', 
      text: 'กรุณาสแกน QR Code จากโต๊ะของคุณใหม่อีกครั้งครับ',
  ...
  // โหลดเมนูอาหารเฉพาะที่พร้อมขาย
  isLoading.value = true
  const { data: mData } = await supabase.from('menus').select('*').eq('status', 'Available')
  if (mData) menus.value = mData
  isLoading.value = false
}
...
// 🚀 ส่งออเดอร์เข้าครัว (รองรับ Real-time)
const submitOrder = async () => {
  if (cart.value.length === 0 || isSubmitting.value || !tableId.value) return
  isSubmitting.value = true

  try {
    // 1. หา Order ของโต๊ะนี้ที่ยังเปิดอยู่ (Open)
    ...
    if (activeOrder) {
      ...
      // อัปเดตสถานะโต๊ะให้เป็น 'Occupied' ทันที
      await supabase.from('tables').update({ status: 'Occupied' }).eq('id', tableId.value)
    }

    // 2. เตรียมรายการอาหารส่งเข้าตาราง order_details (สถานะ Pending)
    ...
    // สำเร็จ! ล้างตะกร้า ปิดหน้าต่าง และสลับไปหน้าบิล
    cart.value = []
    showCartSummary.value = false
    
    Swal.fire({ 
      icon: 'success', 
      title: 'ส่งออเดอร์เข้าครัวแล้ว!', 
      text: 'กรุณารอสักครู่ พนักงานกำลังเตรียมอาหารให้คุณครับ',
      timer: 3000, 
      showConfirmButton: false,
