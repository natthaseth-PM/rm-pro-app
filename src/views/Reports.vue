                ...
                <tbody class="divide-y divide-gray-50">
                  ...
                  <tr v-for="(m, index) in reportData.topMenus" :key="index" class="hover:bg-blue-50/50 transition-colors">
                    <td class="p-4 flex justify-center">
                      <div class="w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm" :class="index === 0 ? 'bg-yellow-100 text-yellow-600' : index === 1 ? 'bg-gray-200 text-gray-600' : index === 2 ? 'bg-orange-100 text-orange-600' : 'text-gray-400'">
...
const donutChartStyle = computed(() => {
  ...
  // ใช้ conic-gradient สร้างกราฟวงกลม สีเขียวคือเงินสด สีม่วงคือ QR
  return {
    background: `conic-gradient(#22c55e 0% ${cashPercentage.value}%, #a855f7 ${cashPercentage.value}% 100%)`
  }
})
...
const generateReport = async () => {
  if (!reportStartDate.value || !reportEndDate.value) {
    Swal.fire({ icon: 'warning', title: 'แจ้งเตือน', text: 'กรุณาเลือกช่วงวันที่ให้ครบถ้วน' })
    return
  }
  if (reportStartDate.value > reportEndDate.value) {
    Swal.fire({ icon: 'error', title: 'ผิดพลาด', text: 'วันที่เริ่มต้นต้องไม่มากกว่าวันที่สิ้นสุด' })
    return
  }
  ...
  try {
    ...
    let cashTotal = 0
    let qrTotal = 0
    let totalDiscount = 0 // ถ้าอนาคตมีระบบบันทึกส่วนลด
    ...
    paidOrders.forEach(order => {
      ...
      netRevenue += amt
      if (order.payment_method === 'Cash') cashTotal += amt
      else qrTotal += amt
    ...
    if (paidOrderIds.length > 0) {
      ...
      detailsData.forEach(item => {
        if (!menuMap[item.menu_name]) {
          menuMap[item.menu_name] = { name: item.menu_name, qty: 0, revenue: 0 }
        }
    ...
    Swal.fire('ผิดพลาด', 'สร้างรายงานล้มเหลว', 'error')
  } finally {
    isReportLoading.value = false
  }
}

// 🖨️ พิมพ์รายงาน A4 (ใช้โครงสร้าง HTML ตามที่คุณส่งมา)
const printReport = () => {
  ...
  let menusHtml = reportData.value.topMenus.map((m, i) => {
    totalMenuQty += m.qty
    totalMenuRevenue += m.revenue
    let rank = (i === 0) ? '🥇 1' : (i === 1) ? '🥈 2' : (i === 2) ? '🥉 3' : (i + 1)
    return `
      <tr class="table-row">
        <td class="text-center font-bold">${rank}</td>
        <td class="font-bold">${m.name}</td>
        <td class="text-center bg-light font-bold">${m.qty}</td>
        <td class="text-right font-bold text-primary">฿${m.revenue.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
      </tr>
    `
  }).join('')
  ...
  if (reportData.value.topMenus.length > 0) {
    menusHtml += `
      <tr style="background-color: #fff7ed; font-weight: 900; border-top: 2px solid var(--primary);">
        <td colspan="2" class="text-right text-primary" style="font-size: 13px;">รวมสินค้าขายดีทั้งหมด (Total)</td>
        <td class="text-center text-primary" style="font-size: 14px;">${totalMenuQty}</td>
        <td class="text-right text-primary" style="font-size: 14px;">฿${totalMenuRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
      </tr>
    `
  }
  ...
  let voidsHtml = reportData.value.voidOrders.map(v => {
    let voidTotal = Number(v.total_amount || 0)
    totalVoidAmount += voidTotal
    return `
      <tr class="table-row void-row">
        <td class="text-sm">${new Date(v.created_at).toLocaleString('th-TH')}</td>
        <td class="font-mono font-bold text-red">#INV-${v.id.split('-')[0].toUpperCase()}</td>
        <td class="text-center font-bold">${v.tables?.table_name || 'ไม่ระบุ'}</td>
        <td class="text-right font-bold text-red">฿${voidTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
      </tr>
    `
  }).join('')
  ...
  if (reportData.value.voidOrders.length > 0) {
    voidsHtml += `
      <tr style="background-color: #fef2f2; font-weight: 900; border-top: 2px solid var(--red);">
        <td colspan="3" class="text-right text-red" style="font-size: 13px;">มูลค่ายกเลิกบิลรวม (Total Void)</td>
        <td class="text-right text-red" style="font-size: 14px;">฿${totalVoidAmount.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
      </tr>
    `
  }
  ...
  const html = `
    <!DOCTYPE html>
    <html><head><title>Sales Report: ${reportData.value.period.start} to ${reportData.value.period.end}</title>
    <link href="https://fonts.googleapis.com/css2?family=Kanit:wght@300;400;700;900&display=swap" rel="stylesheet">
    <style>
      :root { --primary: #f97316; --blue: #3b82f6; --green: #10b981; --red: #ef4444; --dark: #1f2937; --light: #f3f4f6; }
      @media print { 
        @page { size: landscape; margin: 10mm; }
        body { padding: 0; } 
        .grid { border-spacing: 12px 0; margin-left: -12px; } 
      }
      body { font-family: 'Kanit', sans-serif; padding: 20px; color: var(--dark); background: #fff; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      .header { text-align: center; margin-bottom: 20px; border-bottom: 2px dashed #e5e7eb; padding-bottom: 15px; }
      .header h1 { font-size: 24px; font-weight: 900; margin: 0 0 5px 0; color: var(--dark); }
      .header p { margin: 0; color: #6b7280; font-size: 13px; font-weight: bold; }
      .section-title { font-size: 15px; font-weight: 900; color: var(--dark); margin: 20px 0 10px 0; border-left: 4px solid var(--primary); padding-left: 8px; background: #fff7ed; display: inline-block; padding-right: 12px; border-radius: 0 6px 6px 0;}
      .grid { display: table; width: 100%; border-spacing: 12px 0; margin-left: -12px; margin-bottom: 12px; }
      .col { display: table-cell; background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px 15px; box-shadow: 0 2px 4px -1px rgba(0,0,0,0.05); width: 33.33%; }
      .col.half { width: 50%; }
      .card-title { font-size: 11px; color: #6b7280; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px; }
      .card-value { font-size: 22px; font-weight: 900; letter-spacing: -0.5px; }
      .text-blue { color: var(--blue); } .text-green { color: var(--green); } .text-red { color: var(--red); } .text-primary { color: var(--primary); }
      .bg-light { background-color: #f8fafc; }
      table { width: 100%; border-collapse: collapse; margin-bottom: 15px; border: 1px solid #e5e7eb; border-radius: 6px; overflow: hidden; }
      th { background: var(--dark); color: #fff; padding: 8px 10px; font-weight: 700; font-size: 12px; letter-spacing: 0.05em; }
      ...
      .void-table th { background: var(--red); }
      .void-row td { background-color: #fef2f2; color: #991b1b; }
      .text-center { text-align: center; } .text-right { text-align: right; } .font-bold { font-weight: 700; } .font-mono { font-family: monospace; }
      .footer { text-align: center; margin-top: 30px; font-size: 11px; color: #9ca3af; border-top: 1px solid #e5e7eb; padding-top: 15px; font-weight: bold; }
    </style></head><body>

      <div class="header">
        <h1>📊 สรุปยอดขาย (Sales Report)</h1>
        <p><b>ร้าน:</b> ${storeSettings.value.StoreName || 'RM Pro'} | <b>รอบวันที่:</b> ${reportData.value.period.start} ถึง ${reportData.value.period.end}</p>
      </div>

      <div class="section-title">1. สรุปภาพรวม (Executive Summary)</div>
      <div class="grid">
        <div class="col" style="border-top: 4px solid var(--blue); background: #eff6ff;">
          <div class="card-title text-blue">ยอดขายสุทธิ (Net)</div>
          <div class="card-value text-blue">฿${reportData.value.summary.netRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
        </div>
        <div class="col" style="border-top: 4px solid var(--dark);">
          <div class="card-title">จำนวนบิลทั้งหมด</div>
          <div class="card-value">${reportData.value.summary.totalBills} <span style="font-size:13px; color:#6b7280;">บิล</span></div>
        </div>
        <div class="col" style="border-top: 4px solid var(--red); background: #fef2f2;">
          <div class="card-title text-red">ส่วนลดรวม (Discount)</div>
          <div class="card-value text-red">฿${reportData.value.summary.totalDiscount.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
        </div>
      </div>
      <div class="grid">
        <div class="col half" style="border-top: 4px solid var(--green); background: #f0fdf4;">
          <div class="card-title text-green">ชำระด้วยเงินสด (Cash)</div>
          <div class="card-value text-green">฿${reportData.value.summary.cashTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
        </div>
        <div class="col half" style="border-top: 4px solid #a855f7; background: #faf5ff;">
          <div class="card-title" style="color: #a855f7;">โอนเงิน (QR/Transfer)</div>
          <div class="card-value" style="color: #a855f7;">฿${reportData.value.summary.qrTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
        </div>
      </div>
      ...
      <table>
        <thead>
          <tr><th class="text-center" style="width: 10%;">อันดับ</th><th style="width: 50%;">ชื่อเมนู</th><th class="text-center" style="width: 20%;">จำนวน (จาน/แก้ว)</th><th class="text-right" style="width: 20%;">ยอดขาย (฿)</th></tr>
        </thead>
        <tbody>
          ${menusHtml || '<tr><td colspan="4" class="text-center" style="padding: 20px; color:#6b7280; font-weight: bold;">ไม่มีข้อมูลการขายในช่วงเวลานี้</td></tr>'}
        </tbody>
      </table>
      ...
      <table class="void-table">
        <thead>
          <tr><th style="width: 25%;">วัน-เวลาที่ยกเลิก</th><th style="width: 35%;">เลขที่บิล</th><th class="text-center" style="width: 20%;">โต๊ะ</th><th class="text-right" style="width: 20%;">มูลค่า (฿)</th></tr>
        </thead>
        <tbody>
          ${voidsHtml || '<tr><td colspan="4" class="text-center font-bold" style="padding: 20px; color:var(--green); background: #f0fdf4;">ไม่มีการยกเลิกบิลในช่วงเวลานี้ ยอดเยี่ยมมาก! 🎉</td></tr>'}
        </tbody>
      </table>

      <div class="footer">
        พิมพ์เมื่อ: ${new Date().toLocaleString('th-TH')} | พิมพ์โดยระบบบริหารจัดการร้านอาหาร RM Pro
      </div>
