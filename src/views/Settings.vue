  ...
  <div class="h-full flex flex-col bg-gray-50 overflow-hidden relative font-sans text-gray-800">
    ...
    <div class="bg-white px-8 py-6 border-b border-gray-200 shadow-sm z-10 shrink-0">
      ...
      <div class="flex gap-3 mt-6 overflow-x-auto no-scrollbar pb-2">
        ...
        <button @click="settingTab = 'inventory'" :class="['px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border-2', settingTab === 'inventory' ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-500/30' : 'bg-white text-gray-500 border-gray-100 hover:border-blue-300 hover:bg-blue-50']"><i class="fa-solid fa-boxes-stacked mr-2"></i> คลังสินค้า</button>
        <button @click="settingTab = 'members'" :class="['px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border-2', settingTab === 'members' ? 'bg-yellow-500 text-white border-yellow-500 shadow-lg shadow-yellow-500/30' : 'bg-white text-gray-500 border-gray-100 hover:border-yellow-300 hover:bg-yellow-50']"><i class="fa-solid fa-crown mr-2"></i> สมาชิกลูกค้า</button>
        <button @click="settingTab = 'tables'" :class="['px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border-2', settingTab === 'tables' ? 'bg-emerald-600 text-white border-emerald-600 shadow-lg shadow-emerald-500/30' : 'bg-white text-gray-500 border-gray-100 hover:border-emerald-300 hover:bg-emerald-50']"><i class="fa-solid fa-chair mr-2"></i> โต๊ะ</button>
        <button @click="settingTab = 'store'" :class="['px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border-2', settingTab === 'store' ? 'bg-[#1a1a2e] text-white border-[#1a1a2e] shadow-lg' : 'bg-white text-gray-500 border-gray-100 hover:border-gray-400 hover:bg-gray-50']"><i class="fa-solid fa-store mr-2"></i> ข้อมูลร้าน</button>
        <button @click="settingTab = 'users'" :class="['px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border-2', settingTab === 'users' ? 'bg-[#1a1a2e] text-white border-[#1a1a2e] shadow-lg' : 'bg-white text-gray-500 border-gray-100 hover:border-gray-400 hover:bg-gray-50']"><i class="fa-solid fa-users mr-2"></i> ผู้ใช้งาน</button>
        <button @click="settingTab = 'options'" :class="['px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border-2', settingTab === 'options' ? 'bg-[#1a1a2e] text-white border-[#1a1a2e] shadow-lg' : 'bg-white text-gray-500 border-gray-100 hover:border-gray-400 hover:bg-gray-50']"><i class="fa-solid fa-tags mr-2"></i> ตัวเลือก</button>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 md:p-8 relative">
      ...
      <div v-if="settingTab === 'members'" class="animate-[fadeIn_0.3s_ease-out] flex flex-col h-full max-w-7xl mx-auto">
        <div class="flex justify-between items-center mb-6 shrink-0"><h3 class="text-2xl font-black text-gray-800">รายชื่อสมาชิกลูกค้า <span class="text-yellow-600 text-lg">({{ membersList.length }})</span></h3><div class="flex items-center gap-3"><div class="relative"><i class="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i><input v-model="searchMemberQuery" type="text" placeholder="ค้นหาเบอร์ หรือ ชื่อ..." class="pl-10 pr-4 py-3 bg-white border border-gray-200 text-gray-600 font-bold rounded-xl outline-none focus:border-yellow-500 shadow-sm w-56 md:w-64 transition-all"></div><button @click="openMemberModal('add')" class="bg-gradient-to-r from-yellow-400 to-yellow-600 hover:from-yellow-500 hover:to-yellow-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-yellow-500/30 transition-all active:scale-95"><i class="fa-solid fa-user-plus mr-2"></i> เพิ่มสมาชิก</button></div></div>
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 flex-1 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto"><table class="w-full text-left border-collapse"><thead><tr class="bg-yellow-50 text-yellow-800 text-xs uppercase tracking-wider border-b border-yellow-100 sticky top-0 z-10"><th class="p-5 font-bold">เบอร์โทร/ชื่อ</th><th class="p-5 font-bold text-center">ระดับ</th><th class="p-5 font-bold text-right">แต้มสะสม</th><th class="p-5 font-bold text-center">วันหมดอายุสมาชิก</th><th class="p-5 font-bold text-center">จัดการ</th></tr></thead><tbody class="divide-y divide-gray-50"><tr v-if="paginatedMembers.length === 0"><td colspan="5" class="text-center py-12 text-gray-400 font-bold text-lg"><i class="fa-solid fa-users-slash text-4xl block mb-3 opacity-30"></i>ไม่พบข้อมูลสมาชิก</td></tr><tr v-for="m in paginatedMembers" :key="m.phone" class="hover:bg-yellow-50/30 transition-colors"><td class="p-5"><div class="font-black text-gray-800 text-xl tracking-wider">{{ m.phone }}</div><div class="font-bold text-gray-500 text-sm mt-0.5"><i class="fa-solid fa-user mr-1 text-gray-300"></i>{{ m.name }}</div></td><td class="p-5 text-center"><span v-if="m.tier === 'Gold'" class="px-3 py-1.5 bg-gradient-to-r from-yellow-400 to-yellow-600 text-white text-[10px] font-black rounded-lg shadow-sm tracking-widest"><i class="fa-solid fa-crown mr-1"></i> GOLD</span><span v-else-if="m.tier === 'Silver'" class="px-3 py-1.5 bg-gradient-to-r from-gray-400 to-gray-600 text-white text-[10px] font-black rounded-lg shadow-sm tracking-widest"><i class="fa-solid fa-medal mr-1"></i> SILVER</span><span v-else class="px-3 py-1.5 bg-gray-100 text-gray-600 text-[10px] font-black rounded-lg border border-gray-200 tracking-widest">MEMBER</span></td><td class="p-5 font-black text-blue-600 text-2xl text-right">{{ Number(m.points).toLocaleString() }} <div class="text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-wide">หมดอายุ: {{ formatDate(m.points_expiry_date) }}</div></td><td class="p-5 text-center font-bold text-sm" :class="isExpired(m.expiry_date) ? 'text-red-500 bg-red-50 rounded-lg' : 'text-gray-600'">{{ formatDate(m.expiry_date) }}</td><td class="p-5 text-center space-x-2"><button @click="openMemberModal('edit', m)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:bg-blue-500 hover:border-blue-500 hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-pen"></i></button><button @click="deleteMember(m)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:bg-red-500 hover:border-red-500 hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-trash"></i></button></td></tr></tbody></table></div><div class="p-5 border-t border-gray-100 bg-gray-50 flex items-center justify-between shrink-0"><span class="text-sm text-gray-500 font-bold">หน้า {{ currentPageMember }} จาก {{ totalMemberPages }}</span><div class="flex gap-2"><button @click="currentPageMember--" :disabled="currentPageMember === 1" class="px-5 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50 shadow-sm transition-all">ก่อนหน้า</button><button @click="currentPageMember++" :disabled="currentPageMember === totalMemberPages" class="px-5 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50 shadow-sm transition-all">ถัดไป</button></div></div></div>
      </div>

      <div v-if="settingTab === 'tables'" class="animate-[fadeIn_0.3s_ease-out] flex flex-col h-full max-w-5xl mx-auto"><div class="flex justify-between items-center mb-6 shrink-0"><h3 class="text-2xl font-black text-gray-800">จัดการข้อมูลโต๊ะ <span class="text-emerald-600 text-lg">({{ tablesList.length }})</span></h3><button @click="openTableModal('add')" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-emerald-500/30 transition-all active:scale-95"><i class="fa-solid fa-plus mr-2"></i> เพิ่มโต๊ะใหม่</button></div><div class="bg-white rounded-3xl shadow-sm border border-gray-100 flex-1 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto"><table class="w-full text-left border-collapse"><thead><tr class="bg-emerald-50 text-emerald-800 text-xs uppercase tracking-wider border-b border-emerald-100 sticky top-0 z-10"><th class="p-5 font-bold">ชื่อโต๊ะ</th><th class="p-5 font-bold">คำอธิบาย</th><th class="p-5 font-bold text-center">สถานะ</th><th class="p-5 font-bold text-center">จัดการ</th></tr></thead><tbody class="divide-y divide-gray-50"><tr v-for="t in paginatedTables" :key="t.id" class="hover:bg-emerald-50/30 transition-colors"><td class="p-5 font-black text-gray-800 text-xl"><i class="fa-solid fa-chair text-gray-300 mr-2"></i> {{ t.table_name }}</td><td class="p-5 font-bold text-gray-500">{{ t.description || '-' }}</td><td class="p-5 text-center"><span :class="t.status === 'Available' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-orange-50 text-orange-600 border-orange-200'" class="px-3 py-1.5 rounded-full text-[10px] uppercase font-black border tracking-widest">{{ t.status === 'Available' ? 'ว่าง' : 'ไม่ว่าง' }}</span></td><td class="p-5 text-center space-x-2"><button @click="openTableModal('edit', t)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:bg-blue-500 hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-pen"></i></button><button @click="deleteTable(t)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:bg-red-500 hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-trash"></i></button></td></tr></tbody></table></div><div class="p-5 border-t border-gray-100 bg-gray-50 flex items-center justify-between shrink-0"><span class="text-sm text-gray-500 font-bold">หน้า {{ currentPageTable }} จาก {{ totalTablePages }}</span><div class="flex gap-2"><button @click="currentPageTable--" :disabled="currentPageTable === 1" class="px-5 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50 shadow-sm transition-all">ก่อนหน้า</button><button @click="currentPageTable++" :disabled="currentPageTable === totalTablePages" class="px-5 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50 shadow-sm transition-all">ถัดไป</button></div></div></div></div>
      ...
      <div v-if="settingTab === 'store'" class="animate-[fadeIn_0.3s_ease-out] max-w-5xl mx-auto space-y-6">
        <form @submit.prevent="saveStoreSettings" class="space-y-6">
          <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 md:p-10">
            <h3 class="text-2xl font-black text-gray-800 mb-6 flex items-center"><i class="fa-solid fa-store mr-3 text-gray-400"></i> ข้อมูลพื้นฐานร้านค้า</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ชื่อร้าน</label><input v-model="editSettings.StoreName" type="text" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-800 font-bold text-gray-800 focus:bg-white transition-all"></div>
              <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">สาขา</label><input v-model="editSettings.Branch" type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-800 font-bold text-gray-800 focus:bg-white transition-all"></div>
              <div class="md:col-span-2"><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ที่อยู่</label><input v-model="editSettings.Address" type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-800 font-bold text-gray-800 focus:bg-white transition-all"></div>
              <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">เบอร์โทรศัพท์</label><input v-model="editSettings.Phone" type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-800 font-bold text-gray-800 focus:bg-white transition-all"></div>
              <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">เลขประจำตัวผู้เสียภาษี</label><input v-model="editSettings.TaxID" type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-800 font-bold text-gray-800 focus:bg-white transition-all"></div>
              <div class="md:col-span-2"><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ข้อความลงท้ายใบเสร็จ (Footer)</label><input v-model="editSettings.FooterText" type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-800 font-bold text-gray-800 focus:bg-white transition-all"></div>
            </div>
          </div>

          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 md:p-10">
              <h4 class="text-xl font-black text-gray-800 mb-6 flex items-center"><i class="fa-solid fa-money-check-dollar mr-3 text-blue-500"></i> ระบบรับเงิน (QR)</h4>
              <div class="flex gap-3 mb-6">
                <label class="flex-1 cursor-pointer group"><input type="radio" v-model="editSettings.PaymentMode" value="PromptPay" class="peer sr-only"><div class="p-4 rounded-2xl border-2 border-gray-200 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all text-center"><i class="fa-solid fa-qrcode text-2xl mb-2 transition-colors" :class="editSettings.PaymentMode === 'PromptPay' ? 'text-blue-500' : 'text-gray-400'"></i><p class="font-bold text-sm" :class="editSettings.PaymentMode === 'PromptPay' ? 'text-blue-700' : 'text-gray-600'">พร้อมเพย์ (Auto)</p></div></label>
                <label class="flex-1 cursor-pointer group"><input type="radio" v-model="editSettings.PaymentMode" value="BankQR" class="peer sr-only"><div class="p-4 rounded-2xl border-2 border-gray-200 peer-checked:border-purple-500 peer-checked:bg-purple-50 transition-all text-center"><i class="fa-solid fa-image text-2xl mb-2 transition-colors" :class="editSettings.PaymentMode === 'BankQR' ? 'text-purple-500' : 'text-gray-400'"></i><p class="font-bold text-sm" :class="editSettings.PaymentMode === 'BankQR' ? 'text-purple-700' : 'text-gray-600'">ภาพนิ่ง (Manual)</p></div></label>
              </div>
              <div v-if="editSettings.PaymentMode === 'PromptPay'">
                <label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">เบอร์โทร / เลขบัตร ปชช. (PromptPay)</label>
                <input v-model="editSettings.PromptPayID" type="text" class="w-full bg-blue-50 border border-blue-200 rounded-xl px-4 py-3.5 outline-none focus:border-blue-500 font-black text-blue-700 text-xl tracking-wider">
              </div>
              <div v-else class="space-y-5">
                <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ลิงก์รูปภาพ QR</label><input v-model="editSettings.BankQRImageURL" type="url" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-purple-500 font-bold text-gray-800 text-sm"></div>
                <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ชื่อบัญชี / ธนาคาร / เลขบัญชี</label><input v-model="editSettings.BankAccountName" type="text" placeholder="ระบุรายละเอียดบัญชี..." class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-purple-500 font-bold text-gray-800 text-sm"></div>
              </div>
            </div>

            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 md:p-10">
              <h4 class="text-xl font-black text-gray-800 mb-6 flex items-center"><i class="fa-solid fa-crown mr-3 text-yellow-500"></i> ตั้งค่าสมาชิก (CRM)</h4>
              <div class="space-y-6">
                <div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ยอดซื้อขั้นต่ำเพื่อรับ 1 แต้ม (บาท)</label><input v-model="editSettings.PointEarnRate" type="number" class="w-full bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3.5 outline-none focus:border-yellow-500 font-black text-yellow-700 text-xl text-center"></div>
                <div class="grid grid-cols-2 gap-5">
                  <div><label class="block text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-wider">ยอดอัปเกรด <span class="text-gray-600">SILVER</span></label><input v-model="editSettings.TierSilverSpent" type="number" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 outline-none font-black text-gray-700 text-center"></div>
                  <div><label class="block text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-wider">ยอดอัปเกรด <span class="text-yellow-600">GOLD</span></label><input v-model="editSettings.TierGoldSpent" type="number" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 outline-none font-black text-yellow-600 text-center"></div>
                </div>
              </div>
            </div>
          </div>

          <div class="flex justify-end mt-4"><button type="submit" class="bg-gray-800 hover:bg-black text-white px-10 py-4 rounded-xl font-black shadow-lg transition-all active:scale-95 text-lg"><i class="fa-solid fa-save mr-2"></i> บันทึกข้อมูลร้านและตั้งค่า</button></div>
        </form>
      </div>

      <div v-if="settingTab === 'users'" class="animate-[fadeIn_0.3s_ease-out] flex flex-col h-full max-w-5xl mx-auto"><div class="flex justify-between items-center mb-6 shrink-0"><h3 class="text-2xl font-black text-gray-800">บัญชีผู้ใช้งานระบบ <span class="text-gray-400 text-lg">({{ usersList.length }})</span></h3><button @click="openUserModal('add')" class="bg-gray-800 hover:bg-black text-white px-6 py-3 rounded-xl font-bold shadow-lg transition-all active:scale-95"><i class="fa-solid fa-user-plus mr-2"></i> เพิ่มผู้ใช้งาน</button></div><div class="bg-white rounded-3xl shadow-sm border border-gray-100 flex-1 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto"><table class="w-full text-left border-collapse"><thead><tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-100 sticky top-0 z-10"><th class="p-5 font-bold">ชื่อผู้ใช้</th><th class="p-5 font-bold text-center">ตำแหน่ง (Role)</th><th class="p-5 font-bold">สิทธิ์การเข้าถึง</th><th class="p-5 font-bold text-center">จัดการ</th></tr></thead><tbody class="divide-y divide-gray-50"><tr v-for="u in paginatedUsers" :key="u.id" class="hover:bg-gray-50 transition-colors"><td class="p-5 font-black text-gray-800 text-lg"><i class="fa-solid fa-circle-user text-gray-300 mr-2"></i> {{ u.username }}</td><td class="p-5 text-center"><span class="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-gray-100 text-gray-600 border border-gray-200">{{ u.role }}</span></td><td class="p-5 text-[10px] font-bold text-gray-500 flex flex-wrap gap-1.5"><span v-for="page in (u.allowed_pages ? u.allowed_pages.split(',') : [])" :key="page" class="bg-gray-50 border border-gray-200 px-2.5 py-1 rounded-md uppercase tracking-wider">{{ page }}</span></td><td class="p-5 text-center space-x-2"><button @click="openUserModal('edit', u)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:bg-blue-500 hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-pen"></i></button><button @click="deleteUser(u)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:bg-red-500 hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-trash"></i></button></td></tr></tbody></table></div><div class="p-5 border-t border-gray-100 bg-gray-50 flex items-center justify-between shrink-0"><span class="text-sm text-gray-500 font-bold">หน้า {{ currentPageUser }} จาก {{ totalUserPages }}</span><div class="flex gap-2"><button @click="currentPageUser--" :disabled="currentPageUser === 1" class="px-5 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50 transition-all shadow-sm">ก่อนหน้า</button><button @click="currentPageUser++" :disabled="currentPageUser === totalUserPages" class="px-5 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50 transition-all shadow-sm">ถัดไป</button></div></div></div></div>

      <div v-if="settingTab === 'options'" class="animate-[fadeIn_0.3s_ease-out] max-w-6xl mx-auto"><h3 class="text-2xl font-black text-gray-800 mb-6">จัดการข้อมูลตัวเลือก <span class="text-gray-400 text-lg">(Dropdown Options)</span></h3><div class="grid grid-cols-1 md:grid-cols-2 gap-8"><div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[550px]"><div class="bg-[#fff7ed] p-6 border-b border-[#ffedd5]"><h4 class="font-black text-[#ea580c] flex items-center mb-4 text-lg"><i class="fa-solid fa-layer-group mr-2"></i> หมวดหมู่เมนู (Category)</h4><form @submit.prevent="addQuickOption('Category')" class="flex gap-2"><input v-model="quickCategory" type="text" placeholder="พิมพ์ชื่อหมวดหมู่ใหม่..." class="flex-1 px-4 py-3 rounded-xl border border-[#ffedd5] outline-none focus:ring-2 focus:ring-[#f97316]/20 text-sm font-bold"><button type="submit" :disabled="!quickCategory" class="bg-[#f97316] text-white px-5 py-3 rounded-xl font-bold hover:bg-[#ea580c] disabled:bg-[#fdba74] transition-colors"><i class="fa-solid fa-plus"></i></button></form></div><div class="flex-1 overflow-y-auto p-6 space-y-3 bg-gray-50"><div v-if="categoryOptions.length === 0" class="text-center text-gray-400 py-6 text-sm font-bold">ไม่มีข้อมูล</div><div v-for="opt in categoryOptions" :key="opt.id" class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center group"><span class="font-black text-gray-800 text-lg">{{ opt.option_value }}</span><button @click="deleteOption(opt)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-400 hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors"><i class="fa-solid fa-trash-can"></i></button></div></div></div><div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[550px]"><div class="bg-[#f0f9ff] p-6 border-b border-[#dbeafe]"><h4 class="font-black text-[#2563eb] flex items-center mb-4 text-lg"><i class="fa-solid fa-id-badge mr-2"></i> ตำแหน่งพนักงาน (Role)</h4><form @submit.prevent="addQuickOption('Role')" class="flex gap-2"><input v-model="quickRole" type="text" placeholder="พิมพ์ชื่อตำแหน่งใหม่..." class="flex-1 px-4 py-3 rounded-xl border border-[#dbeafe] outline-none focus:ring-2 focus:ring-[#3b82f6]/20 text-sm font-bold"><button type="submit" :disabled="!quickRole" class="bg-[#3b82f6] text-white px-5 py-3 rounded-xl font-bold hover:bg-[#2563eb] disabled:bg-[#93c5fd] transition-colors"><i class="fa-solid fa-plus"></i></button></form></div><div class="flex-1 overflow-y-auto p-6 space-y-3 bg-gray-50"><div v-if="roleOptions.length === 0" class="text-center text-gray-400 py-6 text-sm font-bold">ไม่มีข้อมูล</div><div v-for="opt in roleOptions" :key="opt.id" class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center group"><span class="font-black text-gray-800 text-lg">{{ opt.option_value }}</span><button @click="deleteOption(opt)" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 text-gray-400 hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors"><i class="fa-solid fa-trash-can"></i></button></div></div></div></div></div>

    </div>

    <div v-if="showMenuModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl p-8 md:p-10 animate-[fadeIn_0.2s_ease-out]">
        <h2 class="text-3xl font-black text-gray-800 mb-6 flex items-center"><div class="w-12 h-12 bg-primary/10 text-primary rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-burger"></i></div>{{ menuForm.action === 'add' ? 'เพิ่มเมนูใหม่' : 'แก้ไขข้อมูลเมนู' }}</h2>
    ...
    <div v-if="showMemberModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md p-8 md:p-10 animate-[fadeIn_0.2s_ease-out]">
        <h2 class="text-3xl font-black text-gray-800 mb-6 flex items-center"><div class="w-12 h-12 bg-yellow-50 text-yellow-500 rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-crown"></i></div>{{ memberForm.action === 'add' ? 'เพิ่มสมาชิกใหม่' : 'แก้ไขสมาชิก' }}</h2>
        <form @submit.prevent="submitMemberForm" class="space-y-5">
          ...
          <div class="grid grid-cols-2 gap-4 bg-gray-50 p-5 rounded-2xl border border-gray-200">
    ...
    <div v-if="showUserModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg p-8 md:p-10 animate-[fadeIn_0.2s_ease-out]">
        <h2 class="text-3xl font-black text-gray-800 mb-6 flex items-center"><div class="w-12 h-12 bg-gray-100 text-gray-800 rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-user-shield"></i></div>{{ userForm.action === 'add' ? 'เพิ่มผู้ใช้งาน' : 'แก้ไขผู้ใช้งาน' }}</h2>
        <form @submit.prevent="submitUserForm" class="space-y-5">
          ...
          <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200">
            <label class="block text-sm font-black text-gray-700 mb-4"><i class="fa-solid fa-lock-open mr-2 text-green-500"></i> สิทธิ์การเข้าถึงหน้าต่างๆ (Permissions)</label>
            <div class="grid grid-cols-2 gap-4 text-sm font-bold text-gray-600">
    ...
    <div v-if="showInventoryModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4"><div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md p-8 md:p-10 animate-[fadeIn_0.2s_ease-out]"><h2 class="text-3xl font-black text-gray-800 mb-6 flex items-center"><div class="w-12 h-12 bg-blue-50 text-blue-500 rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-box"></i></div>{{ inventoryForm.action === 'add' ? 'เพิ่มสินค้าลงคลัง' : 'แก้ไขข้อมูลสินค้า' }}</h2><form @submit.prevent="submitInventoryForm" class="space-y-5"><div><label class="block text-xs font-bold text-gray-400 mb-1 uppercase tracking-wider">ชื่อสินค้า / วัตถุดิบ</label><input v-model="inventoryForm.item_name" type="text" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-blue-500 font-bold text-gray-800 text-lg"></div><div v-if="inventoryForm.action === 'add'"><label class="block text-xs font-bold text-gray-400 mb-1 uppercase tracking-wider">จำนวนเริ่มต้น</label><input v-model="inventoryForm.stock_qty" type="number" required class="w-full bg-blue-50 border border-blue-200 rounded-xl px-4 py-3.5 outline-none focus:border-blue-500 font-black text-blue-600 text-2xl text-center"></div><div class="grid grid-cols-2 gap-5"><div><label class="block text-xs font-bold text-gray-400 mb-1 uppercase tracking-wider">หน่วยนับ</label><input v-model="inventoryForm.unit" type="text" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-blue-500 font-bold text-gray-800 text-center"></div><div><label class="block text-[10px] font-bold text-red-500 mb-1 uppercase leading-tight tracking-wider">เตือนเมื่อต่ำกว่า</label><input v-model="inventoryForm.low_alert" type="number" required class="w-full bg-red-50 border border-red-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-500 font-black text-red-600 text-center text-xl"></div></div><div class="flex gap-4 pt-4 border-t border-gray-100 mt-4"><button type="button" @click="showInventoryModal = false" class="flex-1 py-4 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 text-lg">ยกเลิก</button><button type="submit" class="flex-1 py-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl shadow-lg shadow-blue-500/30 active:scale-95 transition-all text-lg"><i class="fa-solid fa-save mr-2"></i> บันทึก</button></div></form></div></div>
    
    <div v-if="showAddStockModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4"><div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 md:p-10 animate-[fadeIn_0.2s_ease-out] text-center"><div class="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center text-3xl mx-auto mb-5 shadow-inner"><i class="fa-solid fa-boxes-packing"></i></div><h2 class="text-3xl font-black text-gray-800 mb-1 tracking-tight">เติมสต๊อกสินค้า</h2><p class="text-gray-500 font-bold mb-6 text-lg">{{ inventoryForm.item_name }}</p><div class="bg-gray-50 p-5 rounded-2xl border border-gray-200 mb-6"><p class="text-sm font-bold text-gray-500">คงเหลือปัจจุบัน: <span class="text-2xl font-black text-gray-800">{{ inventoryForm.stock_qty }}</span> {{ inventoryForm.unit }}</p></div><form @submit.prevent="submitAddStock" class="space-y-5"><div><label class="block text-xs font-black text-green-600 mb-2 uppercase tracking-widest">ระบุจำนวนที่ต้องการเติม (+)</label><input v-model="inventoryForm.add_qty" type="number" required min="1" class="w-full bg-green-50 border border-green-200 rounded-xl px-4 py-4 outline-none focus:border-green-500 font-black text-green-700 text-center text-4xl shadow-inner"></div><div class="flex gap-3 pt-4"><button type="button" @click="showAddStockModal = false" class="flex-1 py-4 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-colors text-lg">ยกเลิก</button><button type="submit" class="flex-1 py-4 bg-green-500 hover:bg-green-600 text-white font-black rounded-xl shadow-lg shadow-green-500/30 active:scale-95 transition-all text-lg"><i class="fa-solid fa-plus mr-1"></i> เพิ่มสต๊อก</button></div></form></div></div>
    
    <div v-if="showLogModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4"><div class="bg-gray-50 rounded-[2.5rem] shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]"><div class="bg-white p-8 flex justify-between items-center shadow-sm z-10"><h2 class="text-2xl font-black text-gray-800 flex items-center"><div class="w-10 h-10 bg-gray-100 text-gray-800 rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-clock-rotate-left"></i></div> ประวัติเข้า-ออกคลังสินค้า</h2><button @click="showLogModal = false" class="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 hover:bg-red-100 hover:text-red-500 text-xl transition-colors shadow-inner"><i class="fa-solid fa-xmark"></i></button></div><div class="flex-1 overflow-y-auto p-6 md:p-8"><div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden"><table class="w-full text-left border-collapse"><thead><tr class="bg-gray-800 text-white text-xs uppercase tracking-wider border-b border-gray-100 sticky top-0 z-10"><th class="p-5 font-bold">วัน-เวลา</th><th class="p-5 font-bold">ชื่อสินค้า</th><th class="p-5 font-bold">เหตุการณ์</th><th class="p-5 font-bold text-center">จำนวน</th><th class="p-5 font-bold text-center">คงเหลือ</th></tr></thead><tbody class="divide-y divide-gray-50"><tr v-if="inventoryLogs.length === 0"><td colspan="5" class="text-center py-10 text-gray-400 font-bold">ไม่มีประวัติการทำรายการ</td></tr><tr v-for="log in paginatedLogs" :key="log.id" class="hover:bg-gray-50 transition-colors"><td class="p-5 text-gray-500 font-medium text-sm">{{ new Date(log.created_at).toLocaleString('th-TH') }}</td><td class="p-5 font-bold text-gray-800 text-lg">{{ log.item_name }}</td><td class="p-5 text-gray-600 font-bold"><span class="bg-gray-100 px-3 py-1 rounded-lg text-xs">{{ log.action }}</span></td><td class="p-5 text-center font-black text-2xl" :class="Number(log.qty_change) > 0 ? 'text-green-500' : (Number(log.qty_change) < 0 ? 'text-red-500' : 'text-gray-500')">{{ log.qty_change > 0 ? '+' : '' }}{{ log.qty_change }}</td><td class="p-5 text-center font-black text-blue-600 text-xl">{{ log.remaining }}</td></tr></tbody></table></div></div><div class="p-6 border-t border-gray-200 bg-white flex items-center justify-between shrink-0"><span class="text-sm text-gray-500 font-bold">หน้า {{ currentPageLog }} จาก {{ totalLogPages }}</span><div class="flex gap-2"><button @click="currentPageLog--" :disabled="currentPageLog === 1" class="px-5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50">ก่อนหน้า</button><button @click="currentPageLog++" :disabled="currentPageLog === totalLogPages" class="px-5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-50">ถัดไป</button></div></div></div></div>
    
    <div v-if="showTableModal" class="absolute inset-0 bg-[#1f2937]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4"><div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 md:p-10 animate-[fadeIn_0.2s_ease-out]"><h2 class="text-3xl font-black text-gray-800 mb-6 flex items-center"><div class="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center mr-3"><i class="fa-solid fa-chair"></i></div>{{ tableForm.action === 'add' ? 'เพิ่มโต๊ะใหม่' : 'แก้ไขข้อมูลโต๊ะ' }}</h2><form @submit.prevent="submitTableForm" class="space-y-5"><div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">ระบุชื่อโต๊ะ</label><input v-model="tableForm.table_name" type="text" required class="w-full bg-emerald-50/50 border border-emerald-200 rounded-xl px-4 py-4 outline-none focus:border-emerald-500 font-black text-2xl text-emerald-800 text-center shadow-inner" placeholder="พิมพ์ชื่อโต๊ะ..."></div><div><label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">คำอธิบาย</label><input v-model="tableForm.description" type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-emerald-500 font-bold text-gray-800 text-center" placeholder="เช่น ชั้น 2, ริมหน้าต่าง"></div><div class="flex gap-4 pt-4 border-t border-gray-100 mt-6"><button type="button" @click="showTableModal = false" class="flex-1 py-4 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-colors text-lg">ยกเลิก</button><button type="submit" class="flex-1 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl shadow-lg shadow-emerald-500/30 active:scale-95 transition-all text-lg"><i class="fa-solid fa-save mr-2"></i> บันทึก</button></div></form></div></div>
